...
static AP_BoardConfig board_config; // 实例化
...
GCS_Dummy _gcs; // 实例化                                                 //gcs stands for Ground Control Station
...
void setup()
{
    ...
    board_config.init(); // 前面已经实例化了是为了这里的使用，用于初始化，板子
    ...
    serial_manager.init(); // 前面已经实例化了是为了这里的使用，用于初始化，串口管理器
    gps.init();            // 同上
}
 ...
 */
void print_latlon(AP_HAL::BetterStream *s, int32_t lat_or_lon); // 经纬度latlon  //声明
void print_latlon(AP_HAL::BetterStream *s, int32_t lat_or_lon)  // 定义
{
...
void loop()
{
    ...
    if (last_msg_ms != gps.last_message_time_ms())
    {
        ...
        // gps.last_message_time_ms()：GPS 库提供的函数，返回最新一条 GPS 消息的时间戳（毫秒）；
        last_msg_ms = gps.last_message_time_ms();
        // 把最新的 GPS 消息时间赋值给静态变量，下次循环用这个值做对比。

        // Acquire location
        const Location &loc = gps.location();
        // Location：GPS 库的结构体，存储经纬度、高度等定位信息；
